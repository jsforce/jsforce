...
Query.prototype.destroy = function(type, callback) {
  ...
  return this.pipe(batch).then(null, function(err) {
    if (err.name === 'ClientInputError') { return []; } // if batch input receives no records
    throw err;
  }).thenCall(callback);
};
...
Query.prototype.update = function(mapping, type, callback) {
  ...
  return this.pipe(updateStream).pipe(batch).then(null, function(err) {
    if (err.name === 'ClientInputError') { return []; } // if batch input receives no records
    throw err;
  }).thenCall(callback);
};
