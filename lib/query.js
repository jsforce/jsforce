...
Query.prototype.include = function(childRelName, conditions, fields, options) {
  ...
  var childQuery = new SubQuery(this._conn, this, {
    table: childRelName,
  ...
  this._config.includes.push(childQuery);
  return childQuery;
};
...
Query.prototype._expandFields = function(callback) {
  ...
  var includes = this._config.includes || [];
  async.waterfall([
    function(cb) {
      if (self._parent) {
        findRelationTable(table, cb);
      } else {
        cb(null, table);
      }
    },
    function(table, cb) {
      async.parallel([
        ...
        function(cb) {
          async.forEach(includes, function(childQuery, cb) {
  ...
  function findRelationTable(rname, callback) {
    ...
    async.waterfall([
      ...
      function(sobject, cb) {
        ...
        if (childRelation) {
          cb(null, childRelation.childSObject);
        } else {
          cb({ message: 'No child relationship found: ' + rname });
        }
        cb(null, 'Contact');
      }
    ], callback);
        ...
        function(sobject, cb) {
          if (fpath.length > 1) {
            ...
            if (rfield) {
              ...
              async.waterfall([
                function(cb) {
                  expandAsteriskField(rtable, fpath.join('.'), cb);
                },
                function(fpaths, cb) {
                  cb(null, _.map(fpaths, function(fpath) { return rname + '.' + fpath; }));
                }
              ], cb);
            } else {
              cb(null, []);
...
Query.prototype.toString = function() {
  return this._soql || SOQLBuilder.createSOQL(this._config);
};
