...
var Query = module.exports = function(conn, config, locator) {
  ...
  var outputStream = this._outputStream = new RecordStream.Serializable();
...
/**
 * Pipe queried records to record stream.
 * Automatically resume query if paused.
 *
 * @param {RecordStream} rstream - A writable record stream
 * @returns {RecordStream}
 */
Query.prototype.pipe = function(rstream) {
 ...
 */
Query.prototype.stream = function(type, options) {
  var stream = this._outputStream.stream.apply(this._outputStream, arguments);
  this._resume();
  return stream;
};
