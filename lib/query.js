...
var Query = module.exports = function(conn, config, locator) {
  ...
  var outputStream = this._outputStream = new RecordStream();
  this.on('record', function(record) { outputStream.write(record); });
  this.on('end', function() { outputStream.end(); });
};

inherits(Query, events.EventEmitter);
 ...
 * @private
 */
Query.prototype._pause = function() {
  this._paused = true;
};
 ...
 * @private
 */
Query.prototype._resume = function() {
  if (this._closed) {
    throw new Error("resuming already closed query");
  }
  if (!this._paused) { 
    return;
 ...
 * @private
 */
Query.prototype._close = function() {
  this._pause();
  this._closed = true;
  this.emit('end', this);
};
...
/**
 * Delegate record stream to pipe queried records.
 * Also resume query if paused.
 * 
 * @param {RecordStream} writable - A writable record stream
 * @returns {RecordStream}
 */
Query.prototype.pipe = function(writable) {
  var dest = this._outputStream.pipe.apply(this._outputStream, arguments);
  this._resume();
  return dest;
};

/**
 * Delegate to record stream and apply map function to queried records
 *
 * @param {RecordMapFunction} fn - Record mapping function
 * @returns {RecordStream}
 */
Query.prototype.map = function(fn) {
  return this._outputStream.map.apply(this._outputStream, arguments);
};

/**
 * Delegate to record stream and apply filter function to queried records
 *
 * @param {RecordFilterFunction} fn - Record filtering function
 * @returns {RecordStream}
 */
Query.prototype.filter = function(fn) {
  return this._outputStream.filter.apply(this._outputStream, arguments);
};
