...
CacheEntry.prototype = new events.EventEmitter();
...
/**
 * Enable the function to store cache for callbacked result,
 * It also creates an alternative cache-first version of the function.
 */
Cache.prototype.enableCaching = function(obj, fnName, options) {
  options = options || {};
  options.namespace = options.namespace || fnName;
  var fn = obj[fnName];
  obj[fnName] = this.enableResponseCaching(fn, obj, options);
  obj[fnName + "$"] = this.enableCacheFirstCaching(fn, obj, options);
};
 ...
 */
Cache.prototype.enableResponseCaching = function(fn, scope, options) {
  var cache = this;
  options = options || {};
  return function() {
...
/**
 * Enable caching for async call fn to lookup the response cache first, then invoke original.
 */
Cache.prototype.enableCacheFirstCaching = function(fn, scope, options) {
  var cache = this;
  options = options || {};
  var $fn = function() {
    var args = Array.prototype.slice.apply(arguments);
    ...
    if (!_.isFunction(callback)) { // if callback is not given in last arg, return cached result (immediate).
      var res = entry.get();
      if (!res) { throw new Error('Function call result is not cached yet.'); }
