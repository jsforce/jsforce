...
function authorize(clientName, callback) {
  ...
  print('URL: ' + authzUrl);
  const cb = (...args) => process.nextTick(() => void callback(...args));
  Promise.all([
    openUrl(authzUrl),
    waitCallback(oauth2Config.redirectUri, state).then((params) => {
      ...
      return conn.authorize(params.code);
    }),
  ]).then(() => {
    ...
    saveCurrentConnection();
  }).then(() => void cb(null), (err) => void cb(err));
}
 ...
 * @private
 */
function waitCallback(serverUrl, state) {
  return new Promise((resolve, reject) => {
    if (serverUrl.startsWith('http://localhost:')) {
      ...
      const msg = 'Copy & paste authz code passed in redirected URL: ';
      promptMessage(msg, (err, code) =>
        err
        ? reject(err)
        : resolve({ code: decodeURIComponent(code), state: state }));
    }
  });
}
