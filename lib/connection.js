 ...
 * @param {Callback.<Request>} [backdoor] - Callback function that receives request object.
 * @returns {Promise.<UserInfo>}
 */
Connection.prototype.authorize = function(code, callback, backdoor) {
  var self = this;
  var logger = this._logger;

  return this.oauth2.requestToken(code, null, backdoor).then(function(res) {
    var userInfo = parseIdUrl(res.id);
    self.initialize({
      instanceUrl : res.instance_url,
      accessToken : res.access_token,
