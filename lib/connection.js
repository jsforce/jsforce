...
var Connection = module.exports = function(options) {
  ...
  var batchCacheOptions = {
    key: function(types, index) {
      var keys = [];
      for (var i = index; i < index + MAX_BATCH_REQUESTS && i < types.length; i++) {
        keys.push('describe.' + types[i]);
      }
      return types;
    }
  };
 ...
 * @param {String|DescribeSObjectOptions} type - SObject Type
 ...
 * @param {String[]} types - SObject Type
 * @param {number} next - the next index in the types array to process (max batch size is 25)
 * @param {Callback.<DescribeSObjectResult[]>} [callback] - Callback function
 * @returns {Promise.<DescribeSObjectResult[]>}
 */
/**
...
Connection.prototype.batchDescribe = Connection.prototype.batchDescribeSObjects = function (
  ...
  var url = [this._baseUrl(), "composite/batch"].join("/");
  const batchSize = MAX_BATCH_REQUESTS;
  var batchRequests = [];
  for (var i = next; i < next + batchSize && i < types.length; i++) {
    batchRequests.push({
      method: "GET",
      url: ["v" + this.version, "sobjects", types[i], "describe"].join("/")
    });
  }
  return this.request({
    method: "POST",
    ...
    headers: {
      ...
      if (response.results) {
        var i = next;
        for (var subResp of response.results) {
          if (Array.isArray(subResp.result)) {
            if (subResp.result[0].errorCode && subResp.result[0].message) {
              this._logger.error(
                `Error: ${subResp.result[0].errorCode} ${subResp.result[0].message} - ${types[i]}`
              );
            }
          } else {
