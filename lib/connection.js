...
var Connection = module.exports = function(options) {
  ...
  // if crossdomain proxy is set (for browser client)
  if (options.proxyUrl) {
...
/**
 * Convert usual request parameters to AJAX Proxy-acceptable request paramters
 * @private
 */
function convertToProxyRequestParams(proxyUrl, params) {
  var proxyParams = {
    method: params.method,
    url: proxyUrl + '?' + Date.now() + "." + ("" + Math.random()).substring(2),
    headers: {
...
Connection.prototype._httpRequest = function(params, callback) {
  params = this.proxyUrl ? convertToProxyRequestParams(this.proxyUrl, params) : params;
...
Connection.prototype._request = function(params, callback, options) {
  ...
  return this._httpRequest(params, function(err, response) {
...
Connection.prototype.identity = function(callback) {
  ...
  if (idUrl) {
    return this._request(idUrl).thenCall(callback);
  } else {
    var self = this;
    return this._request(this._baseUrl()).then(function(res) {
      return self._request(res.identity);
    }).then(function(res) {
      self.userInfo = {
        id: res.user_id,
