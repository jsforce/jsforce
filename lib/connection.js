...
/**
 * Retrieve specified records using external id
 *
 * @param {String} type - SObject Type
 * @param {String} extIdField - A name of external id field
 ...
 * @returns {Promise.<Record|Array.<Record>>}
 */
Connection.prototype.retrieveByExtId = function(type, extIdField, id, options, callback) {
  if (typeof options === 'function') {
    callback = options;
    options = {};
  }
  ...
  if (!id) {
    return Promise.reject(new Error('Invalid external record ID. Specify valid external record ID value'));
  }
  var url = [ this._baseUrl(), "sobjects", type, extIdField, id ].join('/');
  if (options.fields) {
