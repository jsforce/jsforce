  ...
  var batchCacheOptions = {
    key: function(options) {
      ...
      var typesToFetch = types.length > MAX_BATCH_REQUESTS 
        ...
        : types;
      var keys = [];
      typesToFetch.forEach(type => keys.push('describe.' + type));
      return keys;
    }
  };
...
Connection.prototype.doBatchDescribeRequests = function (
  ...
  typesToFetch.forEach(type => {
    batchRequests.push({
      method: "GET",
      url: ["v" + this.version, "sobjects", type, "describe"].join("/")
    });
  });
  return this.request({
    method: "POST",
    ...
    headers: {
      ...
      if (typesRemaining.length > 0 && autofetch) {
        return this.doBatchDescribeRequests({ types: typesRemaining, autofetch }).then(
          results => { 
            results.forEach(result => sobjects.push(result)); 
