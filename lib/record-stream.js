...
/**
 * CSVStream (extends extends ReadableStream implements OutputRecordStream)
 */
...
CSVStream.prototype.send = function(record) {
  if (!this.wroteHeaders) {
    if (!this.headers) {
      ...
      this.headers = _.keys(headers);
    }
    this.emit("data", CSV.arrayToCSV(this.headers) + "\n");
  ...
  this.emit("data", CSV.arrayToCSV(row) + "\n");
};
...
CSVStream.prototype.destroy = 
CSVStream.prototype.destroySoon = function() {
  this.sendable = false;
  this.readable = false;
};
