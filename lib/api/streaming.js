 ...
 * @returns {Subscription} - Faye subscription object
 */
Topic.prototype.subscribeWithExtension = function(extension, listener) {
  return this._streaming.subscribeWithExtension(this.name, extension, listener);
};
...
/** @private **/
Streaming.prototype._getFayeClient = function(channelName, extension) {
  ...
  if (!this._fayeClients || !this._fayeClients[clientType]) {
    ...
    if (typeof extension !== "undefined") {
      this._fayeClients[clientType].addExtension(extension)
    }
 ...
 * @returns {Subscription} - Faye subscription object
 */
Streaming.prototype.subscribeWithExtension = function(name, extension, listener) {
  var channelName = name.indexOf('/') === 0 ? name : '/topic/' + name;
  var fayeClient = this._getFayeClient(channelName, extension);
  return fayeClient.subscribe(channelName, listener);
};
