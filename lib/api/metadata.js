...
Metadata.prototype.deployRecentValidation = function(validationId, callback) {
  const res = this._invoke("deployRecentValidation", { validationId: validationId });
  return new DeployResultLocator(this, res.then(resValue => ({ id: resValue }))).thenCall(callback);
  }
/*--------------------------------------------*/
