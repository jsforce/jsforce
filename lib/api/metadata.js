...
Metadata.prototype.checkStatus = function(ids, callback) {
  ...
  var res = this._invoke("checkStatus", { asyncProcessId: ids }).then(function(result){
    console.info('Status: ' + result.state + ' - ' + new Date().getTime());
    return result;
  });
  return new AsyncResultLocator(this, res, isArray).thenCall(callback);
};
    ...
    self.check().then(function(results) {
      ...
      for (var i=0, len=resultArr.length; i<len; i++) {
        ...
        if (result && !result.done) {
          console.info('Status: ' + result.state + ' - ' + now - startTime );
