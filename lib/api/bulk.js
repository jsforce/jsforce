 ...
 * @param {Callback.<Array.<RecordResult>>} [callback] - Callback function
 * @returns {Promise.<Array.<RecordResult>|Array.<Bulk~BulkQueryResult>>}
 */
Batch.prototype.retrieve = function(callback) {
  ...
  return job.info().then(function(jobInfo) {
    ...
    if (job.operation === 'query') {
      ...
      results = _.map(_.isArray(results) ? results : [ results ], function(resultId) {
        return { id: resultId };
      });
    } else {
...
/**
 * Fetch query result as a stream
 */
 ...
 *
 */
Bulk.prototype.query = function(soql, callback) {
  var m = soql.replace(/\([\s\S]+\)/g, '').match(/FROM\s+(\w+)/i);
  if (!m) {
    throw new Error("No sobject type found in query, maybe caused by invalid SOQL.");
  }
  var type = m[1];
  return this.load(type, "query", soql, callback);
};
