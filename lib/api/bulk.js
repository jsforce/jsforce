...
var CONTENT_TYPE_ENUM = Object.freeze({
  CSV: 'CSV',
  XML: 'XML',
  JSON: 'JSON',
  ZIP_CSV: 'ZIP_CSV',
 ...
 *  Possible values 'CSV', 'XML', 'JSON', 'ZIP_CSV', 'ZIP_XML', or 'ZIP_JSON'. Defaults to 'CSV'.
 * @param {String} [jobId] - Job ID (if already available)
 */
var Job = function(bulk, type, operation, options, jobId) {
  var contentTypes = Object.keys(CONTENT_TYPE_ENUM)
    .map(function(key) {
      return CONTENT_TYPE_ENUM[key];
    });
  ...
  this.options.contentType = this.options.contentType || CONTENT_TYPE_ENUM.CSV;
  this.id = jobId;
  this.state = this.id ? 'Open' : 'Unknown';
  this._batches = {};

  if (contentTypes.indexOf(this.options.contentType) === -1) {
    throw new Error('Not valid contentType. Values ' + contentTypes.join(', '));
  }
};
...
Batch.prototype._createRequestStream = function() {
  ...
  switch(batch.job.options.contentType) {
    case CONTENT_TYPE_ENUM.XML:
      contentType = 'text/xml';
      break;
    case CONTENT_TYPE_ENUM.JSON:
      contentType = 'application/json';
      break;
    case CONTENT_TYPE_ENUM.ZIP_CSV:
      contentType = 'zip/csv';
      break;
    case CONTENT_TYPE_ENUM.ZIP_XML:
      contentType = 'zip/xml';
      break;
    case CONTENT_TYPE_ENUM.ZIP_JSON:
      contentType = 'zip/json';
      break;
    case CONTENT_TYPE_ENUM.CSV:
    default:
