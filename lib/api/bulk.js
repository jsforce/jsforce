...
Job.prototype.info = function(callback) {
  ...
  if (!this._jobInfo || (this.options.pkChunking && !this._jobInfo.numberBatchesTotal)) {
...
Job.prototype.retrieveBatches = function(callback) {
  ...
  return self.list().then((batchInfoList) => {
    return Promise.all(batchInfoList.map(function(batchInfo) {
      if (batchInfo.state === 'NotProcessed') return Promise.resolve();
  ...
  var poll = function() {
    ...
    job.info().then(function(jobInfo) {
      self.check(function(err, res) {
        const batchesComplete = jobInfo.numberBatchesTotal && jobInfo.numberBatchesTotal === jobInfo.numberBatchesCompleted;
        if (err) {
          self.emit('error', err);
        } else {
          if (res.state === "Failed") {
