...
SOAP.prototype.getResponseBody = function(response) {
  ...
  if(this._conn.debuggingHeader !== undefined){
    var Body = lookupValue(body, [ /:Envelope$/, /:Body$/, /.+/ ]); 
    var Header = lookupValue(body, [ /:Envelope$/, /:Header$/, /.+/ ]); 
    var newResponse = {
        body: Body,
        header: Header
    };
    this._conn.debuggingHeader = undefined;
    return newResponse;
  }
  else {
...
SOAP.prototype._createEnvelope = function(message) {
  ...
  if (conn.debuggingHeader) {
    header.DebuggingHeader = conn.debuggingHeader;
  }
  var envelope = [
