 ...
 */
SOAP.prototype.invoke = function(method, args, schema, callback, customHeaders) {
  ...
  return this.request({
    ...
    soapHeaders: customHeaders ? customHeaders : null,
    message: message
  }).then(function(res) {
 ...
 * @private
 */
SOAP.prototype._createEnvelope = function(message, customHeaders) {
  var header = {};
  var conn = this._conn;
  if (conn.accessToken) {
  ...
  if (customHeaders) {
    for (var property in customHeaders) {
      if (customHeaders.hasOwnProperty(property)) {
        header[property] = customHeaders[property];
      }
    }
  }
  return [
