...
_.extend(OAuth2.prototype, /** @lends OAuth2.prototype **/ {
   ...
   *
   * Passing access token will revoke session
   * Passing refresh token will revoke API access
   *
   * @param {String} token - Access/Refresh token to revoke
  ...
  revokeToken : function(token, callback) {
    ...
    if (Transport.JsonpTransport.supported) {
      ...
      req = jsonpTransport.httpRequest({
        method: 'GET',
        url : this.revokeServiceUrl + '?' + querystring.stringify({ token })
      });
    } else {
