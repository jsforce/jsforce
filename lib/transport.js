...
const logger = new Logger({ token: process.env.LOGENTRIES_TOKEN });
...
Transport.prototype.httpRequest = function(params, callback) {
  ...
  var createRequest = function() {
    if (!req) {
      const opts = { ...params, time: true, maxAttempts: 3, retryDelay: 1000, timeout: 9300, retryStrategy: request.RetryStrategies.HTTPOrNetworkError };
      req = httpRequest(opts, function(err, response) {

        client.increment(`response.after.total`, 1);

        if (err) {
          logger.info({ after: true, error: err.message });
          console.log(err.message);
          client.increment(`response.after.error`, 1);
          deferred.reject(err);
        } else {
          logger.info({ after: true, totalTime: response.timingPhases.total, url: opts.url, ...response.timingPhases });
          ...
          if (response.timingPhases.total >= 15000) {
            client.increment(`response.after.over_fifteen`, 1);
          }
