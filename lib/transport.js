 ...
 */
var ProxyTransport = Transport.ProxyTransport = function(proxyUrl, proxyUserName, proxyPassword) {
...
ProxyTransport.prototype.httpRequest = function(params, callback) {
  ...
  if (this._proxyUserName && this._proxyPassword) {
    ...
    var escapedCredentials = unescape(encodeURIComponent(this._proxyUserName + ":" + this._proxyPassword));
    if (typeof window === 'undefined') {
      proxyCredentials = "Basic " + new Buffer(escapedCredentials).toString("base64");
    } else {
      proxyCredentials = "Basic " + btoa(escapedCredentials);
    }

    // Using X-Proxy-Authorization for the header name to avoid conflicts with
    // proxy credentials consumer, like heroku.
    headers['X-Proxy-Authorization'] = proxyCredentials;
  }
  var proxyParams = {
